...
export function highlight(code) {
  ...
  while ((m = TOKEN_RE.exec(code))) {
    ...
    if (comment) out += `<span class="tok-com">${escapeHtml(comment)}</span>`;
      ...
      // Function call if followed by "(" -> color as fn; keyword if reserved.
      const after = code.slice(m.index + full.length);
      if (KEYWORDS.has(ident)) out += `<span class="tok-kw">${escapeHtml(ident)}</span>`;
      else if (/^\s*\(/.test(after)) out += `<span class="tok-fn">${escapeHtml(ident)}</span>`;
