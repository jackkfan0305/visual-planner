...
/** Compute an LCS line diff -> array of { type: "ctx"|"add"|"del", text }. */
...
export async function renderDiff(container, slug, fromRev, toRev) {
  ...
  const banner = el("div", { class: "diff-banner" }, [
    ...
    " → ",
    el("b", { text: `rev ${toRev}` }),
    el("button", { class: "exit", text: "Exit diff", onClick: () => exitDiff(container) }),
  ]);
  ...
  if (changes === 0) {
    ...
    const html = lines
      .map((l) => {
        const cls = CLS[l.type];
        const g = GUTTER[l.type];
        return `<span class="diff-line ${cls}"><span class="gutter">${g}</span>${escapeHtml(l.text)}</span>`;
      })
      .join("");
