...
let menuEl = null;
let cbs;
export function initRevisions(refs, callbacks) {
    cbs = callbacks;
...
function openMenu(overlayRoot) {
    ...
    revs.forEach((r) => {
        ...
        list.append(el("button", { class: "rev-item" + (isCurrent ? " current" : ""), type: "button", onClick: () => select(r.rev) }, [
    ...
    const compareBtn = el("button", {
        ...
        onClick: () => {
            if (comparing)
                cbs.onSelect(store.currentRev);
            else
                cbs.onCompare(store.currentRev - 1, store.currentRev);
            closeMenu();
        },
    });
...
function select(rev) {
    closeMenu();
    if (rev !== store.currentRev || store.compareBase != null)
        cbs.onSelect(rev);
}
function closeMenu() {
    if (menuEl)
