...
function header(openCount) {
  ...
  const next = (store.manifest?.latest || cur) + 1;
  return (
...
export function buildFeedback() {
  ...
  if (store.tone === "concise") {
    let s = `Revise \`${revFile(store.currentRev)}\` into \`${revFile((store.manifest?.latest || store.currentRev) + 1)}\` to address these comments:\n\n`;
    open.forEach((c, i) => {
  ...
  const toneBtn = (tone, label) =>
    el("button", {
      ...
      onClick: () => {
        ...
        modalEl.querySelectorAll(".tone-toggle button").forEach((b) =>
          b.classList.toggle("on", b.textContent.toLowerCase() === store.tone)
        );
      },
    });
