...
export function openCopyModal(overlayRoot: HTMLElement, showToast: (msg: string) => void): void {
  ...
  const toneBtn = (tone: "detailed" | "concise", label: string): HTMLElement =>
    el("button", {
      ...
      onClick: () => {
        ...
        modal.querySelectorAll(".tone-toggle button").forEach((b) =>
  ...
  const modal = el("div", { class: "modal", "data-cc-ui": "", onClick: (e: Event) => e.stopPropagation() }, [
