...
function splitHeading(text: string, fallbackIndex: number): { num: string; title: string } {
...
function isAlert(blockquote: MarkedToken): Alert | null {
  ...
  const cls: "note" | "warn" =
    kind === "WARN" || kind === "WARNING" || kind === "CAUTION" || kind === "IMPORTANT" ? "warn" : "note";
  return { cls, title: m[2].trim(), body: m[3].trim() };
}

function renderCallout(alert: Alert): HTMLElement {
...
function renderTree(code: string): HTMLElement {
  const html = code
    .split("\n")
    .map((line) => {
...
export function renderPlan(container: HTMLElement, markdown: string): void {
  ...
  while (i < tokens.length) {
    ...
    if (head.type === "heading" && head.depth === 2) {
      ...
      sections.append(
        el("section", { class: "card", dataset: { section: title } }, [
